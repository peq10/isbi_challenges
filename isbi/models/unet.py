...
class Unet(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.encoder_convs = torch.nn.ModuleList(
            [unet_parts.double_conv(*x) for x in zip(channels_in, channels_out)]
        )
        ...
        self.up_convs = torch.nn.ModuleList(
            [
                torch.nn.ConvTranspose2d(*x, kernel_size=2, stride=2, padding=0)
                for x in zip(channels_out, channels_in)
            ][:0:-1]
        )
        ...
        self.decoder_convs = torch.nn.ModuleList(
            [unet_parts.double_conv(*x) for x in zip(channels_out, channels_in)][:0:-1]
        )
    ...
    def forward(self, image):
        ...
        for idx, (crop, up_conv_layer, decoder_conv_layer) in enumerate(
            zip(self.crops, self.up_convs, self.decoder_convs)
        ):
            # upsample and concatenate with encoded result for passover connections
            self.intermediates.append(
                torch.cat(
                    [
                        self.intermediates[7 - 2 * idx][..., crop:-crop, crop:-crop],
                        up_conv_layer(self.intermediates[-1]),
                    ],
                    dim=1,
                )
            )
