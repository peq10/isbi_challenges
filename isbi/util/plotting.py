...
def show_error(labels, predictions, alpha=0.5):
    labels = np.squeeze(labels)
    predictions = np.squeeze(predictions)
    error = (labels != predictions).astype(int)

    fig, ax = plt.subplots()
    ax.imshow(np.squeeze(labels), cmap="Greys_r")

    overlay = error[..., None] * np.array([255, 0, 0, int(255 * alpha)])
    ax.imshow(overlay)
